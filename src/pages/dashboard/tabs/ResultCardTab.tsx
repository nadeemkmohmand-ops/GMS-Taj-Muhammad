...
<div class="dmc">
<div class="header">
  <div class="logo-wrap">${logoUrl?`<img src="${logoUrl}" alt="Logo"/>`:`<span class="logo-txt">GMS</span>`}</div>
  <div class="school-center">
    <div class="school-name">${school?.school_name||"GMS Taj Muhammad"}</div>
    <div class="school-addr">${school?.address||"Taj Muhammad, District Mohmand, KPK"}</div>
    <div class="dmc-badge">DETAIL MARKS CERTIFICATE</div>
    <div class="emis-line">EMIS: ${school?.emis_code||"66013"}${school?.phone?` | Ph: ${school.phone}`:""}</div>
  </div>
...
<div class="footer">
  <div class="fl">${school?.school_name||"GMS Taj Muhammad"} | ${school?.address||"Taj Muhammad, District Mohmand, KPK"}</div>
                  ...
                  <motion.div key={r.id} initial={{opacity:0,scale:0.98}} animate={{opacity:1,scale:1}}
                    ...
                    <div className="px-5 py-3 flex items-center gap-3 flex-wrap border-b border-border">
                      ...
                      {r.position && r.position<=10 && (
                        <span className="inline-flex items-center gap-1 bg-blue-50 border border-blue-200 text-blue-900 px-3 py-1.5 rounded-xl text-sm font-semibold">
